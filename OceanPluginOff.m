% Ocean Plugin Matlab Style
classdef OceanPluginOff < audioPlugin & matlab.System
    ...
    methods (Access = protected)
        ...
        function setupImpl(plugin, ~)
            ...
            plugin.inputBlock = zeros(plugin.blockSize, 1, 'single');
            ...
            plugin.outputBlock = zeros(plugin.blockSize * plugin.zeroPad);
        ...
        function out = stepImpl(plugin, in)    
            ...
            % If octaver not sused, bypass expensive computation
%             if plugin.Octave == "Off"
%                 out = monoIn;
%             else
            
                % MAYBE ADD 1 to compensate for index starting at 1: blocksize - hopesize + 1
                % Shift buffer for overlap part
                plugin.inputBlock(1:plugin.blockSize - plugin.hopSize) = ...
                    plugin.inputBlock(plugin.hopSize : plugin.hopSize + (plugin.blockSize - plugin.hopSize) - 1);
                
                % Fill buffer with new audio frame/block
                plugin.inputBlock(plugin.blockSize - plugin.hopSize : (plugin.blockSize - plugin.hopSize) ...
                    + plugin.hopSize - 1 ) = monoIn;
                
                % Apply analysis window using vectorisation (element-wise mult)
                plugin.windowedInputBlock = plugin.inputBlock .* plugin.analysisWindow;
                ...
                plugin.outputBlock = real(ifft(reconstrutedAndShiftedSignal, plugin.blockSize * plugin.zeroPad));

                % Window output block with synthesis window
                plugin.windowedOutputBlock = plugin.outputBlock .* plugin.synthesisWindow;
                
                % Mix last overlap-added history hop with first hop of new block
                out = (plugin.windowedOutputBlock(1:plugin.hopSize) + plugin.overlapAddBuffer(1:plugin.hopSize));
                
                % Shift overlap-add
                plugin.overlapAddBuffer(1:plugin.blockSize - plugin.hopSize * 2) = ... 
                    plugin.overlapAddBuffer(plugin.hopSize:plugin.hopSize + (plugin.blockSize - plugin.hopSize * 2) - 1);
                
                % Fill rest of overlapadd buffer with zeros
                % MIGHT NEED TO ADD 1 as block - hop * 2 = 0 if hop = 50% 
                olaIndex = plugin.blockSize - plugin.hopSize * 2;
                plugin.overlapAddBuffer(olaIndex : olaIndex + (plugin.blockSize - plugin.hopSize) - 1 ) ...
                        = zeros(plugin.blockSize - plugin.hopSize, 1, 'single');

                for i = 1 : plugin.blockSize - plugin.hopSize
                    plugin.overlapAddBuffer(i) = plugin.windowedOutputBlock(i + plugin.hopSize);
                end
...
%             end
        ...
        function resetImpl(plugin)
            ...
            reset(OceanPluginOff);

            % reset input block with new block size
            plugin.inputBlock = zeros(plugin.blockSize, 1, 'single');
            ...
            for bin = 2:plugin.nbInputBins
                ...
                if newBin > 0 && newBin < plugin.nbOutputBins
                    ...
                    if newBin >= paddedBin
                        ...
                        cycleShift = mod(plugin.cycleLength - (paddedBin - newBin), cycleIndex);
                    end
